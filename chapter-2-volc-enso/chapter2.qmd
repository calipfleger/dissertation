...
_stats = json.loads(Path("code/stats.json").read_text())
...
::: {.callout-note appearance="simple" icon=false}
📥 **[Download full analysis deck (PPTX)](presentations/VolcanoENSO_Analysis_v4.pptx)** — 41 slides, all figures. To refresh after a pipeline rerun, run `bash sync_figures.sh` from `~/Documents/Claude/Projects/volcano\ enso/`.
:::
...
The analysis is organized around three shared modules (total ~624 lines) that replaced approximately 500 lines of inline duplication across an earlier 28-file codebase. Code lives at `~/Documents/Claude/Projects/volcano enso/`.
...
The key remaining open question is whether the iCESM LME pseudo-coral network preserves sufficient signal-to-noise to detect the preconditioning effect at all. With only n = 4 members and n = 2 eruptions, the sample size is small; the single La Niña-preconditioned Pinatubo member (M2) and the single El Niño member (M1) are each represented by only 1 of 8 pairs. Expanding to additional last-millennium eruptions (Tambora 1815, Krakatoa 1883) and the full 13-member LME would substantially strengthen the preconditioning analysis.

## 5. Conclusions

## 6. Data and code availability

- Analysis code: `~/Documents/Claude/Projects/volcano enso/` (to be published on GitHub; username TBD).
