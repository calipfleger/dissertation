...
::: {.under-review}
The atmospheric-river thread running through this chapter has so far been statistical: [@lunanino2025heresy] and [@diaz2026atmrivers] both establish that AR activity interferes with the canonical ENSO-to-California-precipitation teleconnection, but neither supplies the moisture-source physics that would explain why. [@small2026mjoar] address exactly that gap. Rather than relating ARs to the MJO through the EOF-based Real-time Multivariate MJO index, as earlier work has done, they track MJO large-scale precipitation systems and ARs jointly in space and time using the Large-scale Precipitation Tracking method, across satellite and reanalysis data from 2000 to 2024. The result is a direct connection rather than an index correlation: MJO convection acts as a major water-vapour source for ARs during boreal winter, when MJO precipitation systems extend furthest into the west-central Pacific. This matters for Chapter 3 in a specific way. If the ARs that disrupt the ENSO signal draw their moisture from MJO convection in the west-central Pacific, then a volcanic perturbation to that convective region — the same ITCZ and West Pacific warm-pool response this dissertation examines in Chapter 2 — has a physical pathway to California hydroclimate that does not run through the Niño 3.4 index at all. That is a mechanism the chapter can state and, in principle, test, and it reframes the AR "disruption" result from a statistical nuisance into a second, parallel teleconnection worth tracking on its own terms. ⚠ Bibliographic metadata verified only against search listings — see the bibliography note.
:::

## References (2026-09-02 phase-4)
- Small, C. A., Chen, S. S., & Kerns, B. W. (2026). Madden–Julian Oscillation and atmospheric rivers: New insights on water source and transport for extreme rainfall over the western U.S. *Journal of Geophysical Research: Atmospheres*, 131, e2025JD045573. [10.1029/2025JD045573](https://doi.org/10.1029/2025JD045573)

_Retrieved via WebSearch (semanticSearch substitute — see the 2026-08-18 pipeline log note) · The AGU/Wiley page returned an empty body for the 3rd consecutive run, so metadata is from search listings and is flagged as unconfirmed · Summary generated by AI from the published abstract — verify claims against the full text · Last corpus update: 2026-09-02_
